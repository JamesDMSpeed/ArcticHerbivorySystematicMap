...
#KMZ for Isa
ag1<-spTransform(agzones,alldata_splaea@proj4string)
ag1@data<-data.frame(ObjectID=1:7)
sb1<-subarcbound
sb1@data<-data.frame(ObjectID=0)

studypol<-rbind(ag1,sb1)
studypolLL<-spTransform(studypol,'+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0')
KML(studypolLL,"Data/StudyAreaKML")
